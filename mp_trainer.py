...
class MP_TrainerAM(BaseTrainer):
    ...
    def init_fn(self):
        ...
        # self.train_ds = HabitatDataOfflineMPv2(self.options, config_file=self.options.config_train_file, img_segm=self.options.with_img_segm)
        # self.test_ds = HabitatDataOfflineMPv2(self.options, config_file=self.options.config_val_file, img_segm=self.options.with_img_segm)
    ...
    def train_step(self, input_batch, step_count):
        ...
        pred_map_loss_objects.sum().backward(retain_graph=True)
        ...
        output = {}
        output['maps'] = {
                          'pred_maps_objects':pred_maps_objects.detach(),
                          # 'pred_maps_spatial':pred_maps_spatial.detach(),
                          }
        output['metrics'] = {
                             'pred_map_err_objects': loss_output['pred_map_err_objects'],
                             # 'pred_map_err_spatial': loss_output['pred_map_err_spatial'],
                             }
        output['losses'] = {
                            'pred_map_loss_objects': pred_map_loss_objects,
                            # 'pred_map_loss_spatial': pred_map_loss_spatial,
                            'pred_am_loss': loss_output['pred_am_loss']
                            }
    ...
    def _save_summaries(self, batch, output, save_images, is_train=False):
        ...
        if save_images:
            # input crops
            # color_step_geo_crops = viz_utils.colorize_grid(batch['pred_ego_crops_sseg'], color_mapping=27)
            # self.summary_writer.add_video(prefix+"gifs/input_pred_ego_crops_sseg", color_step_geo_crops, self.step_count, fps=0.25)

            color_step_geo_crops = viz_utils.colorize_grid(batch['step_ego_grid_27'], color_mapping=27)
            self.summary_writer.add_video(prefix+"gifs/input_step_ego_grid_27", color_step_geo_crops, self.step_count, fps=0.25)

            # color_pred_crops_objects = viz_utils.colorize_grid(batch['pred_maps_objects'], color_mapping=27)
            # self.summary_writer.add_video(prefix + "gifs/pred_crops_objects", color_pred_crops_objects, self.step_count,
            #                               fps=0.25)
            ...
            color_pred_crops_objects = viz_utils.colorize_grid(output['maps']['pred_maps_objects'], color_mapping=27)
            ...
            color_gt_crops_objects = viz_utils.colorize_grid(batch['gt_grid_crops_objects'], color_mapping=27)
