...
class CheckpointSaver(object):
    ...
    def save_best_checkpoint(self, models, optimizers, epoch, batch_idx, batch_size, dataset_perm, total_step_count):
        timestamp = datetime.datetime.now()
        # checkpoint_filename = os.path.abspath(os.path.join(self.save_dir, str(epoch) + '_' + str(total_step_count) + '_' + timestamp.strftime('%Y_%m_%d-%H_%M_%S') + '.pt'))
